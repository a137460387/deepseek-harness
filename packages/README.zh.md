...
| [`mcp/`](mcp/README.md) | 把外部 MCP 服务器工具注册到 `ctx.tools` | 产品：稳定 API |
...
| [`extensions/`](extensions/README.md) | agent 运行时自修改：实时插件／服务检查和模型所写插件挂载／卸载（[设计](../.agents/notes/implemented/feature/2026-07-08-self-referential-cordis-toolset.md)）；外加 fork Web UI 扩展（[落位](../.agents/notes/implemented/architecture/2026-08-19-fork-ui-extensions-placement.md)） | 产品：稳定 API |
...
| [`runtime-diagnostics/`](runtime-diagnostics/README.md) | 包运行时不变量注册设施 | 支持：兼容性预期较低 |

...
- **遍历状态**就是这一个内存槽:不跨刷新存活,由会话切换与草稿清空(发送成功)重置。
...
遍历进行中,ArrowUp/ArrowDown 不复查光标(草稿重写后光标位置由引擎决定),因此遍历中的编辑不会打断游标。input-trigger 服务经 `ctx.get` 可选读取,未组合 slash 管线的组合仍有历史召回。

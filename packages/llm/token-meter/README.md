...
`tokenUsage` carries the complete durable log's `uncachedInputTokens`, `outputTokens`, `cacheReadTokens`, and `cacheWriteTokens`, covering both main-loop requests and compaction summarization calls. Usage chunks are counted even when a request later fails; a final assistant-message usage for the same `(turn, step)` replaces that sample instead of double-counting it. Reasoning remains an output subdivision. The single last-sample slot relies on a session-log ordering property: once a later step reports usage, a legal log never reports usage for an earlier step again. A `compaction/summary` event carries no turn/step, so its logged `usage` never enters the replacement logic — it accumulates in full, and a summary without `usage` contributes nothing.
...
- **Summarizer usage is counted only when emitted** — a `compaction/summary` without `usage` (a template or remote summarizer that reported none) contributes nothing to `tokenUsage`, so totals undercount those calls.
- **Auxiliary title generation is not counted** — `session-title-llm` logs no usage, so its calls stay outside every usage fold; recording them is deferred work.

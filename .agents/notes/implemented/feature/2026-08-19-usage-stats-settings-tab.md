...
The settings dialog gains a fifth nav row ("Usage"/使用统计) rendering figures over every session the host lists, including sessions recorded before the plugin existed (history-tail backfill, at most eight calls in flight, completed backfills cached and reused on later refreshes while the baseline still lacks the key, so only the first open pays the fold cost). The donut is whole-history (the range selector scopes the trend chart only); the activity calendar caps at 53 weeks while every headline figure still counts the full history; usage accrued while the tab stays open appears on the next refresh. The counts inherit `tokenUsage`'s scope, so auxiliary title generation (`session-title-llm`) stays uncounted — recorded in the package README's Known Limitations alongside the five-hue chart palette.

...
6. **验收（逐门真实退出码 + 白名单对照 + 自查）**：每个门单独执行并记录真实退出码，禁止管道吞码。Windows 本机已知失败白名单仅三类：symlink EPERM（本机无 symlink 权限）、POSIX 信号/进程树语义失败、负载抖动（隔离重跑可绿）；对照法：把当前失败文件集与白名单基线逐文件比对，白名单外的失败文件必须先归因才能收尾。doc-sync 唯一许可残留是 doc-site 门的 symlink EPERM 环境项。hygiene 的 node-next types 检查同因目录 symlink EPERM 失败（脚本以 symlinkSync 搭建临时 node_modules，EPERM 即抛、tsc 未运行）。另：hygiene 的 vendor rescope 门带两条过期 exact edit（`knip-logger-console`、`vendoring-cookbook-name-invariant-zh`，登记的宿主上下文已不在文件中——`packages/util/home` 条目等已消失），在基点 `8ddf535ca0` 实证为预存、非环境项，待维护者定夺；该项未清前，hygiene 门预期剩 node-next（环境）与 vendor rescope（预存）两失败。收尾前按验收自查清单逐项核对。

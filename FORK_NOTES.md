...
- **README.md 顶部 fork 横幅**：根 `README.md` 第 1 行（上游标题之前）插入 fork 声明块——声明本仓库为个人维护 fork 并指向上游仓库地址、列出 fork 增量交付（[SECURITY_ADVISORY.md](SECURITY_ADVISORY.md)、[DEPLOY_TUNNEL.md](DEPLOY_TUNNEL.md)、本文件），以「以下为上游 README 原文,本 fork 未改动。」分界；块外零字节改动，上游正文逐字节保持。横幅登记为「上游文件零改动」的唯一例外（[SECURITY_ADVISORY.md](SECURITY_ADVISORY.md) 第 3 节同步引用），根 README 除该横幅外不受 fork 触碰。**合并冲突解决规则**：与 upstream 合并发生冲突时，横幅整体保留在文件最前，其余内容一律以上游版本为准。状态：不适用上报——fork 身份声明，无上游对应修复面。
